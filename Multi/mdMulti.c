...
//Number of events to allocate memory for per particle
#define MAXEVENTS 10
...
event* root;
event** eventpool;
int nempty = 0;     //Number of available events in the pool
...
void init()
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        findlistupdate(p);					//Find the next cell crossing
    }
...
void makeneighborlist(particle* p1, int firsttime)
{
    ...
    if (!firsttime)     //Update events for p1 (unless we are just initializing)
    {

      event* del = p1->cellcrossing;                        //Delete all old events for these particles
      while (del->nextp1 != del) removeevent(del->nextp1); //This includes the old neighborlist update
      while (del->nextp2 != del) removeevent(del->nextp2);
      removeevent(del);
      findlistupdate(p1);           //Find new cellcrossings
      findcollisions(p1, NULL);
    }
}
...
void findcollision(particle* p1, particle* p2)
{
    ...
    if (t < p1->cellcrossing->eventtime && t < p2->cellcrossing->eventtime) createevent(t, p1, p2, 0);
}
...
void findcollisions(particle* p1, particle* notthis)    //All collisions of particle p1
{
    ...
    {
        p2 = p1->neighbors[i];
        if (p2 != notthis) findcollision(p1, p2);
    }
}
...
**************************************************/
void findlistupdate(particle* part)
{
  
    double t = findneighborlistupdate(part);
    createevent(part->t + t, part, part, 8);
}
...
void initeventpool()
{
    ...
    createevent(0, NULL, NULL, 100); //Setup write event
...
void thermostat()
{
    ...
    for (i = 0; i < freq; i++)
    {
        ...
        while (del->nextp2 != del) removeevent(del->nextp2);
        removeevent(del);
        findlistupdate(p);           //Find new cellcrossings
